...
%   2.4 WSOE: definición, algoritmo de minimización, complejidad y por
%       qué escala mal (es el cuello de botella que la tesis ataca).
%       -> FALTA agregar. \cite{wsoe}. La complejidad de acá se contrasta
%          con el O(m log n) de branching bisimilarity en 3.3.
%
% Nota: el \section{Un ejemplo concreto} (TravelAgency) más abajo apunta
% al "capítulo 4" para el benchmark; en la estructura nueva la
% experimentación es el Cap. 6 -> actualizar esa referencia.
% =====================================================================
...
%
% TODO(2.4): \section{WSOE (Weak Synthesis Observational Equivalence)} -- AGREGAR.
%   - Definición de WSOE. \cite{wsoe}.
%   - Algoritmo de minimización por WSOE y su complejidad.
%   - Por qué escala mal -> motiva el reemplazo por branching bisimilarity
%     (Cap. 3) y la demo de equivalencia (Cap. 4). Contrastar esta
%     complejidad con el O(m log n) del algoritmo de Groote et al.
% =====================================================================

\section{Un ejemplo concreto}

Para ejemplificar y tener una noción acerca del tamaño que puede alcanzar el problema de composición y su posterior exploración, presentaremos una versión simplificada del problema \textbf{TravelAgency} que describiremos con mayor detalle posteriormente en el capítulo 4 y será usado para medir la \textit{performance} del algoritmo.
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=.85\textwidth]{capitulos/cap1/img/EjemploTAControladorMTSA.png}
    \caption{Controlador para el problema que usa 2 sub-servicios}
    \label{fig:ejemploTAPlanta}
\end{figure}
